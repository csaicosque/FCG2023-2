...
void initGLFW() {
    ...
    window = glfwCreateWindow(screenWidth, screenHeight, "Círculo OpenGL", nullptr, nullptr);
...
void drawCircle(float centerX, float centerY, float radius, int segments, float r, float g, float b) {
    ...
    for (int i = 0; i <= segments; i++) {
        float theta = 2.0f * 3.14159265359f * static_cast<float>(i) / static_cast<float>(segments);
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Defina a cor do círculo (vermelho)
        float r = 1.0f;
        float g = 0.0f;
        float b = 0.0f;

        drawCircle(centerX, centerY, radius, segments, r, g, b);
